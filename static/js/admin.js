...
function renderCourseCard(course) {
  return `
    <div class="admin-course" data-course-id="${course.course_id}">
      <div class="admin-course-head">
        ...
        <button type="button" class="btn btn-secondary" data-toggle="${course.course_id}">
          View students
        </button>
      </div>
      <div class="student-table-wrap hidden" id="students-${course.course_id}">
...
addCourseForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  const payload = {
    department_id: Number(courseDeptSelect.value),
    level_id: Number(courseLevelSelect.value),
...
async function init() {
  try {
    ...
    if (courseDeptSelect.value) {
      await loadLevelsInto(courseLevelSelect, courseDeptSelect.value, false, true);
    }
    updateCourseContextHint();
    await loadCourses();
  } catch (err) {
