...
async function toggleStudents(button) {
  ...
  try {
        ...
        <tbody>
          ${students
            ...
            <tr>
              ...
              <td>${s.rfid_card_id ? `<span class="badge">${s.rfid_card_id}</span>` : '—'}</td>
              <td><span class="badge">${s.attendance_count}</span></td>
              <td>
                <button type="button" class="btn btn-danger btn-sm" data-delete="${escapeHtml(s.matric_no)}">Delete</button>
              </td>
            </tr>`
            )
            .join("")}
    ...
    panel.querySelectorAll("[data-delete]").forEach((btn) => {
      btn.addEventListener("click", () => deleteStudent(btn.dataset.delete, courseId));
    });
  } catch (err) {
...
async function deleteStudent(matricNo, courseId) {
  if (!confirm(`Are you sure you want to delete student ${matricNo}? This will permanently delete all their data including face samples, embeddings, course registrations, attendance records, and RFID card. This action cannot be undone.`)) {
    return;
  }

  try {
    const res = await fetch(`/students/${matricNo}`, { method: "DELETE" });
    if (!res.ok) {
    ...
    const button = document.querySelector(`[data-toggle="${courseId}"]`);
    if (button) {
      // Toggle to refresh
      button.click();
    ...
    alert(`Student ${matricNo} has been permanently deleted.`);
  } catch (err) {
